    ...
    def __init__(self, dim, act_layer=nn.GELU, params=params):
        ...
        if params["spatial_mixer"]["fc_factor"] > 1:
            self.fc1 = nn.Sequential(
                nn.Conv2d(dim,
                          max(dim // params["spatial_mixer"]["fc_factor"], params["spatial_mixer"]["fc_min_value"]), 1),
                self.act,
                nn.Conv2d(max(dim // params["spatial_mixer"]["fc_factor"], params["spatial_mixer"]["fc_min_value"]),
                          dim, 1)
            )
        else:
        ...
        if params["spatial_mixer"]["useSecondTokenMix"]:
            ...
            if params["spatial_mixer"]["fc_factor"] > 1:
                self.fc2 = nn.Sequential(
                    nn.Conv2d(dim,
                              max(dim // params["spatial_mixer"]["fc_factor"], params["spatial_mixer"]["fc_min_value"]),
                              1),
                    self.act,
                    nn.Conv2d(max(dim // params["spatial_mixer"]["fc_factor"], params["spatial_mixer"]["fc_min_value"]),
                              dim, 1)
...
class BasicBlock(nn.Module):
    def __init__(self, dim, mlp_ratio=4.,
        ...
        self.channel_mixer = ChannelMixer(dim=dim, hidden_dim=mlp_hidden_dim, act_layer=act_layer, drop=drop,
                                          params=params)
...
class FCViT(BaseFormer):
    def __init__(self, arch, **kwargs):
        self.arch = arch
        fcvit_cfg = fcvit_cfgs[self.arch]
        fcvit_params = params.copy()
        fcvit_params["spatial_mixer"]["useSecondTokenMix"] = True
        fcvit_params["spatial_mixer"]["use_globalcontext"] = True
        fcvit_params["spatial_mixer"]["mix_size_1"] = 11
        fcvit_params["spatial_mixer"]["mix_size_2"] = 11
        fcvit_params["spatial_mixer"]["useSpatialAtt"] = False
        fcvit_params["global_context"]["weighted_gc"] = True
        fcvit_params["global_context"]["head"] = 8
        fcvit_params["global_context"]["compete"] = True
        fcvit_params["channel_mixer"]["useDWconv"] = True
        fcvit_params["channel_mixer"]["useChannelAtt"] = False

        layers = fcvit_cfg["layers"]
        embed_dims = fcvit_cfg["embed_dims"]
        mlp_ratios = fcvit_cfg["mlp_ratios"]
        downsamples = fcvit_cfg["downsamples"]
