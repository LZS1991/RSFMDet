...
class MobileOne(nn.Module):
    ...
    def __init__(self,
        ...
        self.stage1 = self._make_stage(int(64 * width_multipliers[0]), num_blocks_per_stage[0],
                                       num_se_blocks=0)
        self.norm_layer1 = nn.LayerNorm(int(64 * width_multipliers[0]))
        self.stage2 = self._make_stage(int(128 * width_multipliers[1]), num_blocks_per_stage[1],
                                       num_se_blocks=0)
        self.norm_layer2 = nn.LayerNorm(int(128 * width_multipliers[1]))
        ...
        self.norm_layer3 = nn.LayerNorm(int(256 * width_multipliers[2]))
        ...
        self.norm_layer4 = nn.LayerNorm(int(512 * width_multipliers[3]))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if 0 in self.out_indices:
            B, C, H, W = x.shape
            x_out = x.view(B, C, -1).permute(0, 2, 1).contiguous()
...
if __name__ == '__main__':
    ...
    model = MobileOne(num_blocks_per_stage=[2, 8, 10, 1], 
