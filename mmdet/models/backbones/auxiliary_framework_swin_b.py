...
from mmdet.models.backbones.swin_b_pis import SwinTransformerPIS
...
from mmdet.models.backbones.swin_b_satlas import SwinTransformerV2Satlas, Norm2d
...
class FLFAuxiliaryFramework(BaseModule):
    ...
    def __init__(self,
        ...
        # self.swin_b = SwinTransformerV2Satlas(num_channels=3, pretrained=foundation_pretrained)
        self.swin_b = SwinTransformerPIS(in_chans=3, pretrained=foundation_pretrained)
        self.swin_b_out_channels = self.swin_b.out_channels
        self.swin_b_drop = DropPath(0.1)
        #  freeze swin_b parameters
        for param in self.swin_b.parameters():
            param.requires_grad = False
        # ==========================End FoundationModel=========================== #

        # ===================Begin Adapter of FoundationModel===================== #
        for i, source_embed_dim in enumerate(self.source_embed_dims):
            swin_b_adapter = nn.Sequential(
                nn.ConvTranspose2d(self.swin_b_out_channels[i][1], source_embed_dim, kernel_size=1, stride=1,
                                   padding=0),
                Norm2d(source_embed_dim),
                nn.GELU(),
            )
            swin_b_adapter_layer_name = 'vit_b_adapter_{}'.format(i)
            self.add_module(swin_b_adapter_layer_name, swin_b_adapter)

            swin_b_fusion = BridgeLayer(embed_dims=source_embed_dim)
            swin_b_fusion_layer_name = 'vit_b_fusion_{}'.format(i)
            self.add_module(swin_b_fusion_layer_name, swin_b_fusion)
