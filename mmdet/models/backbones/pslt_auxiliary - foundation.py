...
from mmcv.cnn import (build_norm_layer, constant_init, normal_init, trunc_normal_init)
...
from mmdet.models.utils import nchw_to_nlc, nlc_to_nchw

from mmdet.models.backbones.auxiliary_framework_vit_b import FLFAuxiliaryFramework, FeatureAggregationExpansionModule


class SepConv2d(BaseModule):
    def __init__(self,
                 in_channels,
                 out_channels,
                 kernel_size,
                 ...
                 dilation=1,
                 init_cfg=None):
        super(SepConv2d, self).__init__(init_cfg=init_cfg)
...
class ChannelSELayer_SA(BaseModule):
    """
    Re-implementation of Squeeze-and-Excitation (SE) block described in:
        *Hu et al., Squeeze-and-Excitation Networks, arXiv:1709.01507*
    """

    def __init__(self, num_channels, reduction_ratio=2, init_cfg=None):
        """
        :param num_channels: No of input channels
        :param reduction_ratio: By how much should the num_channels should be reduced
        """
        super(ChannelSELayer_SA, self).__init__(init_cfg=init_cfg)
...
class ChannelSELayer(BaseModule):
    ...
    def __init__(self, num_channels, reduction_ratio=2, init_cfg=None):
        ...
        :param reduction_ratio: By how much should the num_channels should be reduced
        """
        super(ChannelSELayer, self).__init__(init_cfg=init_cfg)
        num_channels_reduced = num_channels // reduction_ratio
        self.reduction_ratio = reduction_ratio
        self.fc1 = nn.Linear(num_channels, num_channels_reduced, bias=True)
        self.fc2 = nn.Linear(num_channels_reduced, num_channels, bias=True)
...
class Mlp_Light(BaseModule):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., reduce_scale=4,
                 norm_cfg=dict(type='SyncBN', requires_grad=True), init_cfg=None, **kargs):
        super().__init__(init_cfg=init_cfg)
...
class WindowAttention(BaseModule):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
                 input_v=False, input_kv=False, v2k=False, init_cfg=None):
...
class multi_branch_LadderBlock(BaseModule):
    def __init__(self, dim, num_heads, window_size=[3, 5, 7], shift_size=0,
                 ...
                 input_v=False, input_kv=False, has_avg_se=False, init_cfg=None):
        super().__init__(init_cfg=init_cfg)
...
class LadderSABlock(BaseModule):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 ...
                 norm_cfg=dict(type='SyncBN', requires_grad=True), input_v=False, input_kv=False, init_cfg=None):
        super().__init__(init_cfg=init_cfg)
        self.dim = dim
        self.num_heads = num_heads
        self.window_size = window_size
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
                       out_features=out_dim, norm_cfg=norm_cfg)
        self.H = None
        self.W = None
    ...
    def forward(self, x, mask_matrix, in_v=None):
        ...
        x_windows = window_partition(shifted_x,
                                     self.window_size)  # nW*B, window_size, window_size, C   #window_partition
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C)  # nW*B, window_size*window_size, C

        if in_v is not None:
            in_v_windows = window_partition(shift_in_v,
                                            self.window_size)  # nW*B, window_size, window_size, C   #window_partition
            in_v_windows = in_v_windows.view(-1, self.window_size * self.window_size,
...
class PatchMerging(BaseModule):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm, init_cfg=None):
        super().__init__(init_cfg=init_cfg)
        self.dim = dim
...
class BasicLayer_multi_branch(BaseModule):
    ...
    def __init__(self, dim, depth, num_heads, window_size=[3, 5, 7],
                 ...
                 input_v=False, input_kv=False, init_cfg=None):
        ...
        self.blocks = nn.ModuleList([
            multi_branch_LadderBlock(dim=dim,
                                     ...
                                     branch2input_conv=branch2input_conv, pool_same_token=pool_same_token,
                                     pool_less_token=pool_less_token,
                                     max_out=max_out, global_SA=global_SA, extra_conv=extra_conv,
                                     multi_shift=multi_shift, branch_attn=branch_attn,
                                     branch_add=branch_add, multi_shift_first2input=multi_shift_first2input,
                                     dilateConv_branch=dilateConv_branch, sum_cat=sum_cat,
                                     branch_attn_cat=branch_attn_cat, Mlp=Mlp, has_se=has_se,
...
class PatchEmbed_stem_3conv(BaseModule):
    def __init__(self, patch_size=2, in_chans=3, embed_dim=96, norm_layer=None, act_layer=nn.GELU,
                 norm_cfg=dict(type='SyncBN', requires_grad=True), init_cfg=None):
        super().__init__(init_cfg=init_cfg)
...
class PSLTransformerAuxiliary(BaseModule):
    r""" PSLT
    ...
    def __init__(self, pretrain_img_size=224, img_size=224, patch_size=2, in_chans=3,
                 ...
                 ch_shuffle=False, add_dim=False,
                 cross_stage=False, query_branch=False, stage_to_fc=False, branch2input=False,
                 pointwise=False, branch2input_conv=False, pool_same_token=False, pool_less_token=False,
                 max_out=False, aggressive_dim=False, shift_window=True, global_SA=False,
                 extra_conv=False, multi_shift=False, branch_attn=False, branch_add=False,
                 multi_shift_first2input=False, dilateConv_branch=False, sum_cat=False, branch_attn_cat=False,
                 Mlp=Mlp_Light, has_se=False, distilled=False, conv_depths=[3, 2],
                 out_indices=(0, 1, 2, 3), frozen_stages=-1, norm_eval=False,
                 norm_cfg=dict(type='BN', requires_grad=True),
                 input_v=False, init_cfg=None, mobilenet_pretrained=None, foundation_pretrained=None):
        super().__init__(init_cfg=init_cfg)
        ...
        conv_layer = MV2Block_SE(int(embed_dim * 2 ** self.num_conv_layers),
                                 int(embed_dim * 2 ** (self.num_conv_layers + 1)), stride=2, norm_cfg=norm_cfg)
        self.conv_layers.append(conv_layer)
        ...
        source_embed_dims = self.num_features
        ...
        self.mobilenet_fusion_conv_index = [0, 3]
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** (i_layer + self.num_conv_layers + 1)),
                               ...
                               use_checkpoint=use_checkpoint, ch_shuffle=ch_shuffle, add_dim=add_dim,
            ...
            if (i_layer < self.num_layers - 1):
                downsample_layer = PatchMerging(int(embed_dim * 2 ** (i_layer + self.num_conv_layers + 1)),
                                                norm_layer=norm_layer)
    ...
    def forward(self, x):
        ...
        for i, conv_layer in enumerate(self.conv_layers):
            ...
            if 0 in self.out_indices:
                ...
                if i == (conv_depth - 1):
                    ...
                    vit_feature = vit_features[0]
                    vit_b_adapter = getattr(self.auxiliary_net, "vit_b_adapter_{}".format(0))
                    vit_b_fusion = getattr(self.auxiliary_net, "vit_b_fusion_{}".format(0))
            ...
            if 1 in self.out_indices:
                ...
                if i == (conv_depth - 1):
                    # Begin Step 2.2 fuse feature from foundation model
                    vit_feature = vit_features[1]
                    vit_b_adapter = getattr(self.auxiliary_net, "vit_b_adapter_{}".format(1))
                    vit_b_fusion = getattr(self.auxiliary_net, "vit_b_fusion_{}".format(1))
                    vit_feature_adapter = vit_b_adapter(vit_feature)
                    x = vit_b_fusion(x, vit_feature_adapter)
                    # End Step 2.2 fuse feature from foundation model
                    outs.append(x)
        ...
        for i_layer, layer in enumerate(self.layers):
            ...
            x_out, H, W = layer(x, Wh, Ww)

            # Begin Step 2.2 fuse feature from foundation model
            vit_feature = vit_features[i_layer + 2]
            vit_b_adapter = getattr(self.auxiliary_net, "vit_b_adapter_{}".format(i_layer + 2))
            vit_b_fusion = getattr(self.auxiliary_net, "vit_b_fusion_{}".format(i_layer + 2))
            vit_feature_adapter = vit_b_adapter(vit_feature)
            x_out = nlc_to_nchw(x_out, [H, W])
            x_out = vit_b_fusion(x_out, vit_feature_adapter)
            x_out = nchw_to_nlc(x_out)
            # End Step 2.2 fuse feature from foundation model

            if i_layer < len(self.downsample_layers):
                downsample_layer = self.downsample_layers[i_layer]
                x = downsample_layer(x_out, H, W)
                Wh, Ww = (H + 1) // 2, (W + 1) // 2
            else:
                x = x_out
                Wh, Ww = (H + 1) // 2, (W + 1) // 2
            ...
            if (i_layer + self.num_conv_layers) in self.out_indices:
                out = x_out.view(-1, H, W, self.num_features[i_layer + self.num_conv_layers]).permute(0, 3, 1,
                                                                                                        2).contiguous()
                outs.append(out)
...
if __name__ == '__main__':
    ...
    model = PSLTransformerAuxiliary(pretrain_img_size=448, img_size=448, conv_depths=[3, 3], patch_norm=False,
                                    window_size=[7, 7, 7], depths=[10, 3],
                                    num_heads=[4, 8], embed_dim=24,
                                    drop_path_rate=0.2,
                                    pointwise=True, multi_shift=True, has_se=True,
                                    input_v=True, out_indices=(0, 1, 2, 3))
    y = model(x)
    print(y)
